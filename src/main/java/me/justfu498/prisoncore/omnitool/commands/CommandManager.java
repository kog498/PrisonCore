...
import me.justfu498.prisoncore.omnitool.commands.subcommands.AddEnchantCommand;
import me.justfu498.prisoncore.omnitool.commands.subcommands.GiveCommand;
...
public class CommandManager implements CommandExecutor {
    ...
    public CommandManager(PrisonCore plugin) {
        this.plugin = plugin;
        subCommands.add(new GiveCommand(this.plugin));
        subCommands.add(new AddEnchantCommand(this.plugin));
    }
        ...
        if (sender instanceof Player) {
            ...
            if (args.length > 0) {
                    ...
                    if (args[0].equalsIgnoreCase(getSubCommands().get(i).getName())) {
                        if (!getSubCommands().get(i).getPermission().equalsIgnoreCase("none")) {
                            if (player.hasPermission(getSubCommands().get(i).getPermission())) {
                                ...
                                player.sendMessage(utils.color(OmnitoolMessages.NO_PERMISSION.toString()));
                            }
                        } else {
                            getSubCommands().get(i).executeCommand(player, args);
                        }
                ...
                if (player.hasPermission("prisoncore.help.admin")) {
                    for (String line : OmnitoolListMessages.HELP_ADMIN.toStringList()) {
                        player.sendMessage(utils.color(line));
                    }
                    return true;
                }

                for (String line : OmnitoolListMessages.HELP_PLAYER.toStringList()) {
                    player.sendMessage(utils.color(line));
                }
            }
